...
public class OpenDesiresMenuButton extends Button {

    public static final ItemStack ICON = DesiresItems.KINETIC_MECHANISM.asStack();

    public OpenDesiresMenuButton(int x, int y) {
        super(x, y, 20, 20, Components.immutableEmpty(), OpenDesiresMenuButton::click);
    }

    @Override
    public void renderBg(@NotNull PoseStack poseStack, @NotNull Minecraft mc, int mouseX, int mouseY) {
        Minecraft.getInstance().getItemRenderer().renderGuiItem(ICON, x + 2, y + 2);
    }

    public static void click(Button b) {
        ScreenOpener.open(new DesiresMainMenuScreen(Minecraft.getInstance().screen));
    }
    ...
    public static class OpenConfigButtonHandler {
        ...
        public static void onGuiInit(ScreenEvent.Init event) {
            ...
            if (screen instanceof TitleScreen) {
                ...
                rowIdx = DesiresConfigs.client().mainMenuConfigButtonRow.get();
                offsetX = DesiresConfigs.client().mainMenuConfigButtonOffsetX.get();
            } else if (screen instanceof PauseScreen) {
                menu = MenuRows.INGAME_MENU;
                rowIdx = DesiresConfigs.client().ingameMenuConfigButtonRow.get();
                offsetX = DesiresConfigs.client().ingameMenuConfigButtonOffsetX.get();
            } else {
                return;
            }
            ...
            event.getListenersList()
                    ...
                    .findFirst()
                    .ifPresent(w -> toAdd
                            .setValue(new OpenDesiresMenuButton(w.x + offsetX_ + (onLeft ? -20 : w.getWidth()), w.y)));
