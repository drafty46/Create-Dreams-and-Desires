package uwu.lopyluna.create_dd.registry.fabric;
...
public class DesiresEntityTypesImpl {
    ...
    public static <T extends Entity> CreateEntityBuilder<T, ?> register(String name, EntityType.EntityFactory<T> factory, NonNullSupplier<NonNullFunction<EntityRendererProvider.Context, EntityRenderer<? super T>>> renderer, MobCategory group, int clientTrackingRange, int updateFrequency, boolean sendVelocity, boolean immuneToFire, NonNullConsumer<FabricEntityTypeBuilder<T>> propertyBuilder) {
        ...
        return (CreateEntityBuilder<T, ?>) Desires.REGISTRATE.entity(id, factory, group).properties(b -> b.trackRangeBlocks(clientTrackingRange).trackedUpdateRate(updateFrequency).forceTrackedVelocityUpdates(sendVelocity)).properties(propertyBuilder).properties(b -> {
